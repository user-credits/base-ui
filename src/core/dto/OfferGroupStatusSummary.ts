...
export class OfferGroupStatusSummary<K extends IMinimalId> {
  ...
  protected _statusSummary: Status | undefined;
  protected _statusMessage: string = "";
  protected _activeSubscription: ISubscription<K> | undefined;
  ...
  protected calculateStatus(): Status {
    ...
    for (const subscription of this._purchaseGroup) {
      if (subscription.status === "paid" && !this._active)
        console.error(
          "Maybe a corrupted state: subscription ",
          subscription._id,
          " is paid, but no active item found (this is maybe because the subscription is expired or finished the tokens, ignore this if it's the case): ",
      ...
      if (
        // it is not an error to have a pending subscription
        ...
        if (!this.statusSummary)
          // avoid to override another warning
  ...
  protected computeDateSafety(): number {
    ...
    if (!this._active)
      // error, should not happen: a paid subscription must have an active peer
      return -1;
    if (!this._active?.expires)
      // this subscription has no exipry date, we just skip
      return 1;

    const expires = new Date(this._active.expires);
    if (currentDate.getTime() > expires.getTime()) return -1;

    const thresholdDate = new Date(expires.getTime() - delayBeforeExpiry);
    const remainingTime = thresholdDate.getTime() - currentDate.getTime();
  ...
  protected computeTokenSafety(subscription: ISubscription<K>): number {
    ...
    if (!subscription.tokens) return 1;
  ...
  get remainingTokens(): number | null | undefined {
